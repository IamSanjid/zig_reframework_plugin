...
pub const Method = extern struct {
    ...
    pub const Parameter = extern struct {
        ...
        pub inline fn name(self: ParamSelf) [:0]const u8 {
            return std.mem.span(self.raw.name);
        }

        pub inline fn typeDefinition(self: ParamSelf) TypeDefinition {
    ...
    pub inline fn handle(self: Self) API.REFrameworkMethodHandle {
        return self.raw;
    }
    ...
    pub inline fn getFunction(self: Self, FuncT: type, sdk: Verified(API.REFrameworkSDKData, .{ .method = .get_function })) ?*const FuncT {
        const raw = self.getFunctionRaw(sdk) orelse return null;
        return @ptrCast(raw);
    }

    pub inline fn getName(self: Self, sdk: Verified(API.REFrameworkSDKData, .{ .method = .get_name })) ?[:0]const u8 {
        const value = sdk.safe().method.safe().get_name(self.handle()) orelse return null;
        return std.mem.span(value);
    }

    pub inline fn getDeclaringType(self: Self, sdk: Verified(API.REFrameworkSDKData, .{ .method = .get_declaring_type })) ?TypeDefinition {
        const result = sdk.safe().method.safe().get_declaring_type(self.handle());
        return if (result) |value| .{ .raw = @ptrCast(value) } else null;
    }

    pub inline fn getReturnType(self: Self, sdk: Verified(API.REFrameworkSDKData, .{ .method = .get_return_type })) ?TypeDefinition {
        const result = sdk.safe().method.safe().get_return_type(self.handle());
        return if (result) |value| .{ .raw = @ptrCast(value) } else null;
    }

    pub inline fn getNumParams(self: Self, sdk: Verified(API.REFrameworkSDKData, .{ .method = .get_num_params })) u32 {
    ...
    pub inline fn isStatic(self: Self, sdk: Verified(API.REFrameworkSDKData, .{ .method = .is_static })) bool {
        return sdk.safe().method.safe().is_static(self.handle());
    }

    pub inline fn getFlags(self: Self, sdk: Verified(API.REFrameworkSDKData, .{ .method = .get_flags })) u16 {
        return sdk.safe().method.safe().get_flags(self.handle());
    }

    pub inline fn getImplFlags(self: Self, sdk: Verified(API.REFrameworkSDKData, .{ .method = .get_impl_flags })) u16 {
        return sdk.safe().method.safe().get_impl_flags(self.handle());
    }

    pub inline fn getInvokeId(self: Self, sdk: Verified(API.REFrameworkSDKData, .{ .method = .get_invoke_id })) u32 {
